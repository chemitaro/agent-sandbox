...
- [ ] S04: ドキュメント/Devcontainer を整理し、参照が残らない
...
### S04 — ドキュメント/Devcontainer の削除 (必須)
- 対象: AC-003
- 設計参照:
  - 対象IF/API: 該当なし
  - 対象テスト: なし
- このステップで「追加しないこと（スコープ固定）」:
  - README の作り直しはこのステップでは行わない（削除のみ）

#### update_plan（着手時に登録） (必須)
- [ ] `update_plan` に、このステップの作業ステップ（調査/Red/Green/Refactor/品質ゲート/報告/コミット）を登録した

#### 期待する振る舞い（テストケース） (必須)
- Given: 不要なドキュメント/Devcontainer が存在する
- When: 削除と参照残存チェックを行う
- Then: リポジトリ内に `CHANGELOG.md` / `CLAUDE.md` / `README.md` / `.devcontainer` の参照が残らない
- 観測点（UI/HTTP/DB/Log など）: `rg` 検索結果
- 追加/更新するテスト: なし

#### Green（最小実装） (任意)
- 変更予定ファイル:
  - Delete: `CHANGELOG.md`, `CLAUDE.md`, `README.md`, `.devcontainer/`
  - Modify: `.gitignore`
